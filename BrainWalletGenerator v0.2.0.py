# BrainWalletGenerator v0.2.0 by @btcdage
...
def generate_addresses(mnemonic_words, count):
    ...
    output_text += f"私钥: {private_key_wif}\n"
    output_text += f"公钥: {public_key_hex}\n"
    output_text += f"P2PKH地址: {address_p2pkh}\n"
    output_text += f"Bech32私钥: {private_key_bech32}\n"
    output_text += f"Bech32地址: {address_bech32}\n"
...
title_label = ttk.Label(root, text="Brain Wallet Generator V0.2.0/ 比特币脑钱包生成器 V0.2.0", font=("Arial", 16), anchor='center')
...
generate_from_mnemonic_button = ttk.Button(main_frame, text="Generate from mnemonic / 根据助记词生成", command=generate_from_mnemonic)
generate_from_mnemonic_button.pack(pady=5, anchor='center')
...
copy_results_button = ttk.Button(main_frame, text='Copy bitcoin Keys /复制结果', command=lambda: copy_to_clipboard(results_text))
copy_results_button.pack(pady=5, anchor='center')
